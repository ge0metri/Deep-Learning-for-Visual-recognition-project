...
# always ranking
class RankingPermutationDataGenerator(BasePermutationDataGenerator):
    def __init__(self, 
        input, batch_size=64,
        ...
        super(RankingPermutationDataGenerator, self).__init__(
...
def test_datagen_stitched(datagen):
    ...
    print(f"Permutation: {y[random_int]}")
...
def main():
    ...
    for i in number_of_tiles_for_tests:
        ...
        for truth in booleans:
            datagen = RankingPermutationDataGenerator(
                input=[PATH], 
                stitched=True,
                batch_size=1,
